...
export class CommentFormatter {
  formatAnalysis(analysis: PRAnalysisResult): string {
    ...
    // Header
    sections.push('## 🧠 PRBrain Analysis');
    sections.push('');
  ...
  private formatAIDetection(detection: AIDetectionResult): string {
    const emoji = detection.isAIGenerated ? '🤖' : '👨‍💻';
  ...
  private formatDeduplication(dedup: DeduplicationResult): string {
    ...
    if (dedup.isDuplicate) {
      sections.push('### 🚨 Duplicate Detected');
    } else {
      sections.push('### 🔄 Similar PRs/Issues');
    }

    const topSimilar = dedup.similarItems.slice(0, 5);
    
    for (const item of topSimilar) {
      const typeEmoji = item.type === 'pr' ? '🔀' : '📋';
      const statusEmoji = this.getStatusEmoji(item.status);
      const similarity = Math.round(item.similarity * 100);
      
      sections.push(`- ${typeEmoji} #${item.number} "${item.title}" (${similarity}% similar) — ${statusEmoji} ${item.status.toUpperCase()}`);
    }
  ...
  private formatVisionAlignment(vision: VisionAlignmentResult): string {
    ...
    sections.push('### 📐 Vision Alignment');
    
    if (vision.isAligned) {
      sections.push(`✅ Aligned with project goals (${vision.score}/100)`);
    } else {
      sections.push(`❌ Potential misalignment (${vision.score}/100)`);
    }
    ...
    if (misaligned.length > 0) {
      ...
      misaligned.forEach(item => {
        sections.push(`  • ${item.principle}: ${item.reasoning}`);
      });
    }
  ...
  private formatForDiscord(analysis: PRAnalysisResult): string {
    ...
    const ai = analysis.aiDetection.isAIGenerated ? '🤖 AI' : '👨‍💻 Human';
    const confidence = analysis.aiDetection.confidence;
    const quality = analysis.qualityScore.overallScore;
    
    sections.push(`${ai} (${confidence}%) • Quality: ${quality}/10`);
    sections.push('');
  ...
  private formatForSlack(analysis: PRAnalysisResult): string {
    ...
    const ai = analysis.aiDetection.isAIGenerated ? '🤖 AI-Generated' : '👨‍💻 Human-Written';
...
*PRBrain - AI-powered PR analysis*`;
  ...
  formatPartialAnalysis(analysis: Partial<PRAnalysisResult>, errors: string[]): string {
    ...
    sections.push('## 🧠 PRBrain Analysis - Partial Results');
    sections.push('');
    sections.push('⚠️ Some analysis components failed. Results shown are partial.');
    sections.push('');
